...
do
    ...
    local libs = {
        { src = table,     funcs = { "clone", "insert", "remove", "concat", "find", "clear", "sort" } },
        { src = string,    funcs = { "format", "match", "find", "gsub", "sub", "lower", "rep", "split", "byte", "char" } },
        { src = math,      funcs = { "floor", "max", "min", "abs" } },
        { src = coroutine, funcs = { "create", "resume", "wrap", "status" } },
        { src = task,      funcs = { "spawn", "defer", "delay", "wait", "cancel" } },
        { src = os,        funcs = { "clock" } },
        { src = buffer,    funcs = { "len", "readu8", "fromstring", "create" } },
    }

    for _, lib in ipairs(libs) do
        for _, name in ipairs(lib.funcs) do
            if lib.src[name] then
                local prefix = ""
                if lib.src == table then prefix = "table"
                elseif lib.src == string then prefix = "string"
                elseif lib.src == math then prefix = "math"
                elseif lib.src == coroutine then prefix = "coroutine"
                elseif lib.src == task then prefix = "task"
                elseif lib.src == os then prefix = "os"
                elseif lib.src == buffer then prefix = "buffer"
                end
                local key = prefix .. name:sub(1,1):upper() .. name:sub(2)
                ClonedFunctions[key] = clonefunction(lib.src[name])
            end
        end
    end
end
...
function CacheAPI.Invalidate(object)
    ...
    local success = pcall(cache.invalidate, object)
    return success
end

function CacheAPI.IsCached(object)
    if not CacheAPI.IsAvailable() then return nil end
    local success, result = pcall(cache.iscached, object)
    return success and result or nil
end

function CacheAPI.Replace(object, newObject)
    if not CacheAPI.IsAvailable() then return false end
    local success = pcall(cache.replace, object, newObject)
    return success
end
...
local RequiredFunctions = {
    ...
    "setclipboard", "clonefunction", "hookfunction"
}
...
if #MissingFunctions > 0 then
    ...
    if syn and syn.protect_gui then
        ...
        ErrorGui.Parent = Services.CoreGui
    elseif gethui then
...
function Utils.ToHex(color)
    return ClonedFunctions.stringFormat(
        "#%02X%02X%02X",
        ClonedFunctions.mathFloor(color.R * 255),
        ClonedFunctions.mathFloor(color.G * 255),
        ClonedFunctions.mathFloor(color.B * 255)
    )
end
...
function Utils.HighlightLua(code)
    ...
    for _, p in ipairs(patterns) do
        ...
        local currentIdx = 1
        while true do
            local s, e = ClonedFunctions.stringFind(code, p.Pattern, currentIdx)
            if not s then break end
            local captured = ClonedFunctions.stringSub(code, s, e - offset)
            ClonedFunctions.tableInsert(replacements, { s = s, e = e - offset, c = p.Color, t = captured })
            currentIdx = s + 1
        end
    end

    ClonedFunctions.tableSort(replacements, function(a, b) return a.s < b.s end)
    ...
    for _, r in ipairs(replacements) do
        if r.s >= lastPos then
            if r.s > lastPos then
                ClonedFunctions.tableInsert(result, ClonedFunctions.stringSub(code, lastPos, r.s - 1))
            end
            ClonedFunctions.tableInsert(result, '<font color="' .. r.c .. '">' .. r.t .. '</font>')
            lastPos = r.e + 1
        end
    end

    if lastPos <= #code then
        ClonedFunctions.tableInsert(result, ClonedFunctions.stringSub(code, lastPos))
    end

    return ClonedFunctions.tableConcat(result)
end
...
function Utils.Create(className, props)
    ...
    if props.RichText ~= nil then
        pcall(function() instance.RichText = props.RichText end)
        props.RichText = nil
    end
...
local MAX_SERIALIZE_DEPTH = 10
local MAX_STRING_LEN = 10000
local MAX_TABLE_ENTRIES = 500
local MAX_BUFFER_DISPLAY = 512
...
function Utils.Serialize(value, depth, visited)
    ...
    elseif valueType == "string" then
        if #value > MAX_STRING_LEN then
            local truncated = ClonedFunctions.stringSub(value, 1, MAX_STRING_LEN)
            truncated = ClonedFunctions.stringGsub(truncated, '\\', '\\\\')
            truncated = ClonedFunctions.stringGsub(truncated, '"', '\\"')
            truncated = ClonedFunctions.stringGsub(truncated, '\n', '\\n')
            truncated = ClonedFunctions.stringGsub(truncated, '\r', '\\r')
            truncated = ClonedFunctions.stringGsub(truncated, '\t', '\\t')
            truncated = ClonedFunctions.stringGsub(truncated, '%z', '\\0')
            truncated = ClonedFunctions.stringGsub(truncated, '[%c]', function(c)
                return ClonedFunctions.stringFormat('\\x%02X', ClonedFunctions.stringByte(c))
            end)
            return '"' .. truncated .. '..." --[[ truncated, total length: ' .. #value .. ' ]]'
        end
        local escaped = value
        ...
        escaped = ClonedFunctions.stringGsub(escaped, '\t', '\\t')
        escaped = ClonedFunctions.stringGsub(escaped, '%z', '\\0')
        escaped = ClonedFunctions.stringGsub(escaped, '[%c]', function(c)
            return ClonedFunctions.stringFormat('\\x%02X', ClonedFunctions.stringByte(c))
        end)
        return '"' .. escaped .. '"'
        ...
        if isArray then
            for _, v in ipairs(value) do
                ...
                if entryCount > MAX_TABLE_ENTRIES then
                    ClonedFunctions.tableInsert(parts, indent .. '"[... ' .. (count - MAX_TABLE_ENTRIES) .. ' more entries]"')
                    break
                end
            ...
            pcall(function()
                ClonedFunctions.tableSort(keys, function(a, b) return tostring(a) < tostring(b) end)
            end)
    ...
    elseif valueType == "thread" then
        ...
        pcall(function() status = ClonedFunctions.coroutineStatus(value) end)
        return ClonedFunctions.stringFormat('coroutine.create(function() --[[ Status: %s ]] end)', status)

    elseif valueType == "buffer" then
        local size = ClonedFunctions.bufferLen(value)
        if size == 0 then return "buffer.create(0)" end

        local displaySize = ClonedFunctions.mathMin(size, MAX_BUFFER_DISPLAY)
        local hexParts = {}
        local asciiParts = {}
        local lines = {}

        for i = 0, displaySize - 1 do
            local b = ClonedFunctions.bufferReadu8(value, i)
            ClonedFunctions.tableInsert(hexParts, ClonedFunctions.stringFormat("%02X", b))
            if b >= 32 and b <= 126 then
                ClonedFunctions.tableInsert(asciiParts, ClonedFunctions.stringChar(b))
            else
                ClonedFunctions.tableInsert(asciiParts, ".")
            end

            if #hexParts == 16 or i == displaySize - 1 then
                local hexLine = ClonedFunctions.tableConcat(hexParts, " ")
                local asciiLine = ClonedFunctions.tableConcat(asciiParts)
                local offset = ClonedFunctions.stringFormat("%04X", i - #hexParts + 1)
                local padding = ClonedFunctions.stringRep("   ", 16 - #hexParts)
                ClonedFunctions.tableInsert(lines, offset .. "  " .. hexLine .. padding .. "  |" .. asciiLine .. "|")
                hexParts = {}
                asciiParts = {}
            end
        end

        local header = ClonedFunctions.stringFormat("buffer (size=%d)", size)
        if size > MAX_BUFFER_DISPLAY then
            header = header .. ClonedFunctions.stringFormat(" [showing first %d bytes]", MAX_BUFFER_DISPLAY)
        end
        ...
        for i = 0, displaySize - 1 do
            ...
            if b >= 32 and b <= 126 and b ~= 34 and b ~= 92 then
                ClonedFunctions.tableInsert(rawParts, ClonedFunctions.stringChar(b))
            else
                ClonedFunctions.tableInsert(rawParts, ClonedFunctions.stringFormat("\\x%02X", b))
            end
        end
        ...
        return 'buffer.fromstring("' .. ClonedFunctions.tableConcat(rawParts) .. '"' .. ')' .. truncNote .. " --[[\n" .. header .. "\n" .. ClonedFunctions.tableConcat(lines, "\n") .. "\n]]"
    end
...
function Utils.GetCallerInfo(callerFunc, callerScript)
    local info = {
        Source       = "[Unknown]",
        Name         = "[Anonymous]",
        Line         = 0,
    ...
    if callerScript and typeof(callerScript) == "Instance" then
        info.ScriptInstance = callerScript
        info.ScriptPath     = Utils.GetPath(callerScript)
        info.Source         = callerScript:GetFullName()
    end

    if not info.ScriptInstance and callerFunc then
        pcall(function()
            local scriptFromEnv = rawget(getfenv(callerFunc), "script")
            if scriptFromEnv and typeof(scriptFromEnv) == "Instance" then
                info.ScriptInstance = scriptFromEnv
                info.ScriptPath     = Utils.GetPath(scriptFromEnv)
                info.Source         = scriptFromEnv:GetFullName()
            end
        end)
    end
    ...
    pcall(function()
        ...
        for i = 1, 20 do
            local dinfo = debug.getinfo(i)
            if not dinfo then break end

            local src  = dinfo.source or ""
            local what = dinfo.what   or ""

            local isInternal = what == "C"
                or src == "[C]"
                or src == "=[C]"
                or src == "@"
                or src == ""
            ...
            if not isInternal then
                if not foundFirst then
                    ...
                    info.Source      = (info.Source == "[Unknown]") and dinfo.source or info.Source
                ...
                ClonedFunctions.tableInsert(info.StackTrace, {
                    Source = dinfo.source or "?",
                    Name   = dinfo.name   or "?",
                    Line   = dinfo.currentline or 0,
                })

                if #info.StackTrace >= 10 then break end
            end
        end
    end)
...
function LogEntry:GetColor()
    return TypeColors[self.RemoteType] or Theme.Accent
end

function LogEntry:GetTypeShort()
    return TypeShorts[self.RemoteType] or "?"
end
...
function LogEntry:GetDetailedInfo()
    ...
    elseif ClonedFunctions.stringFind(self.RemoteType, "ActorCall_", 1, true) then
        ...
        if self.CallerInfo.StackTrace and #self.CallerInfo.StackTrace > 0 then
            ...
            for i, frame in ipairs(self.CallerInfo.StackTrace) do
                ClonedFunctions.tableInsert(lines, Key("[" .. i .. "] ") .. Value(
                    (frame.Source or "?") .. " : " .. (frame.Name or "?") .. " @ line " .. (frame.Line or 0)
                ))
            end
            ClonedFunctions.tableInsert(lines, "")
        end
    ...
    ClonedFunctions.tableInsert(lines, '<font face="RobotoMono">' .. Value(self:GetScript()) .. '</font>')
...
function LogEntry:CanDecompile()
    return self.CallerInfo.ScriptInstance ~= nil or self.ActorScript ~= nil
end

function LogEntry:GetDecompilableScript()
    return self.CallerInfo.ScriptInstance or self.ActorScript
end
...
function BlockList.new()
    return setmetatable({ Blocked = {} }, BlockList)
end
...
function Logger:GetFilteredGroups()
    ...
    for _, group in pairs(self.Groups) do
        ...
        local passText = self.Filter == ""
            or ClonedFunctions.stringFind(ClonedFunctions.stringLower(group.RemotePath), ClonedFunctions.stringLower(self.Filter), 1, true)
            or ClonedFunctions.stringFind(ClonedFunctions.stringLower(group.Remote.Name), ClonedFunctions.stringLower(self.Filter), 1, true)
...
function UI:CreateFilterButton(filterType, parent)
    ...
    local short = shortNames[filterType] or "?"
...
function UI:CreateGroupItem(group)
    ...
    if colorBar then Utils.Corner(colorBar, UDim.new(0, 2)) end
    ...
    if typeLabel then Utils.Corner(typeLabel, UDim.new(0, 3)) end
    ...
    if countLabel then Utils.Corner(countLabel, UDim.new(0, 3)) end
    ...
    if blockedLabel then Utils.Corner(blockedLabel, UDim.new(0, 3)) end
    ...
    if expandBtn then Utils.Corner(expandBtn, Theme.CornerSmall) end
...
function UI:UpdateGroupItem(group)
    ...
    pcall(function()
        if itemData.BlockedLabel then
            itemData.BlockedLabel.Visible = self.BlockList:IsBlocked(group.Remote)
        end
    end)
...
function UI:CreateSubEntryItem(entry, parent, index)
    ...
    if colorIndicator then Utils.Corner(colorIndicator, UDim.new(0, 1)) end
    ...
    local callerText = callerName
    if entry.RemoteType ~= "OnClientEvent" and not ClonedFunctions.stringFind(entry.RemoteType, "ActorCall_", 1, true) then
        callerText = callerText .. " @ line " .. tostring(entry.CallerInfo.Line or 0)
    end
...
function UI:OpenDetailWindow(entry)
    ...
    entry.Blocked = self.BlockList:IsBlocked(entry.Remote)
    local windowId = "Detail_" .. tostring(entry.Id)

    if self.SubWindows[windowId] then
        pcall(function() self.SubWindows[windowId]:Destroy() end)
    end
    ...
    if btnFrame then
        ...
        self:CreateDetailButton("FIRE REMOTE", btnFrame, function()
            ClonedFunctions.taskSpawn(function()
                ...
                if entry.RemoteType == "OnClientEvent" then
                    if firesignal then
                        pcall(firesignal, rem.OnClientEvent, unpack(args))
                        self:ShowNotification("Signal Fired!", Theme.Success)
                    elseif replicatesignal then
                        pcall(replicatesignal, rem.OnClientEvent, unpack(args))
...
function UI:SetupSubWindowResize(window)
    ...
    pcall(function()
        ...
        local moveConn
        moveConn = Services.UserInputService.InputChanged:Connect(function(input)
            local ok = pcall(function() return window.Parent end)
            if not ok then moveConn:Disconnect() return end
            if resizing and (input.UserInputType == Enum.UserInputType.MouseMovement or
                input.UserInputType == Enum.UserInputType.Touch) then
                local delta = input.Position - startPos
        ...
        endConn = Services.UserInputService.InputEnded:Connect(function(input)
            local ok = pcall(function() return window.Parent end)
            if not ok then endConn:Disconnect() return end
            if input.UserInputType == Enum.UserInputType.MouseButton1 or
                input.UserInputType == Enum.UserInputType.Touch then
                resizing = false
            end
...
function UI:RefreshList()
    for _, itemData in pairs(self.GroupItems) do
        if itemData.Frame then
            pcall(function() itemData.Frame:Destroy() end)
        end
    end
    self.GroupItems = {}

    for _, group in ipairs(self.Logger:GetFilteredGroups()) do
        self:CreateGroupItem(group)
    end
end

function UI:SetupLoggerCallback()
    self.Logger.OnGroupUpdated = function(group)
        self.PendingGroups[group] = true
    end
end
...
function UI:ProcessPendingUpdates()
    ...
    for group in pairs(self.PendingGroups) do
        ...
        pcall(function()
            ...
            local passText = self.Logger.Filter == ""
                or ClonedFunctions.stringFind(ClonedFunctions.stringLower(group.RemotePath), ClonedFunctions.stringLower(self.Logger.Filter), 1, true)
                or ClonedFunctions.stringFind(ClonedFunctions.stringLower(group.Remote.Name), ClonedFunctions.stringLower(self.Logger.Filter), 1, true)
    ...
    for group in pairs(processed) do
...
function UI:SetupWindowDrag(window, header)
    ...
    pcall(function()
        ...
        local conn
        conn = Services.UserInputService.InputChanged:Connect(function(input)
            local ok = pcall(function() return window.Parent end)
            if not ok then conn:Disconnect() return end
            if input == dragInput and dragging then
                local delta = input.Position - dragStart
...
function UI:ToggleMinimize()
    ...
    if self.Minimized then
        if self.Toolbar then Utils.SafeSet(self.Toolbar, "Visible", false) end
        if self.ContentFrame then Utils.SafeSet(self.ContentFrame, "Visible", false) end
        local resizeHandle = self.Main and self.Main:FindFirstChild("ResizeHandle")
        if resizeHandle then Utils.SafeSet(resizeHandle, "Visible", false) end
        ...
        ClonedFunctions.taskDelay(0.2, function()
            if not self.Minimized then
                if self.Toolbar then Utils.SafeSet(self.Toolbar, "Visible", true) end
                if self.ContentFrame then Utils.SafeSet(self.ContentFrame, "Visible", true) end
                local resizeHandle = self.Main and self.Main:FindFirstChild("ResizeHandle")
                if resizeHandle then Utils.SafeSet(resizeHandle, "Visible", true) end
            end
        end)
    end
end
...
function UI:Close()
    ...
    for _, subWindow in pairs(self.SubWindows) do
        pcall(function() subWindow:Destroy() end)
    end
...
function Core:HookActors()
    ...
    if typeof(r1) == "Instance" and r1:IsA("BindableEvent") then
        channel, channelId = r1, r2
    elseif typeof(r2) == "Instance" and r2:IsA("BindableEvent") then
        channel, channelId = r2, r1
    elseif type(r1) == "number" then
        channelId, channel = r1, r2
    elseif type(r2) == "number" then
        channelId, channel = r2, r1
    end

    if not channel or not channelId or type(channelId) ~= "number" then
        return
    end
    ...
    local function handleActorData(data)
        if not selfRef.Active or type(data) ~= "table" then return end
        ...
        if selfRef.BlockList:IsBlocked(remote) then return end
        ...
        selfRef.Logger:Add({
            Remote = remote,
            RemoteType = "ActorCall_" .. remoteType,
...
oldNamecall = hookmetamethod(game, "__namecall", newcclosure(function(self, ...)
    ...
    for _, actor in ipairs(getactors()) do
        pcall(function() run_on_actor(actor, actorHookScript) end)
    end
    ...
    GameRef.DescendantAdded:Connect(function(desc)
        if desc:IsA("Actor") then
            ClonedFunctions.taskDelay(0.5, function()
                pcall(function() run_on_actor(desc, actorHookScript) end)
            end)
        end
    end)
end
...
function Core:HookIncoming()
    ...
    local function ConnectRemote(remote)
        if not remote or selfRef.IncomingConnections[remote] then return end
...
function Core:HookRemotes()
    ...
    local function processCall(self, method, args, isNamecall)
        local class = self.ClassName
        ...
        if isEvent or isFunc or isBindEvent or isBindFunc then
            if selfRef.Active then
                if selfRef.BlockList:IsBlocked(self) then
                    return true
                end

                local remoteType = "Unknown"
                if isEvent then remoteType = class == "UnreliableRemoteEvent" and "UnreliableRemoteEvent" or "RemoteEvent" end
                ...
                local remotePath = Utils.GetPath(self)
                
                local callerFunc = nil
                local callerScript = nil

                pcall(function()
                    if debug and debug.info then
                        callerFunc = debug.info(3, "f")
                    end
                end)

                pcall(function()
                    callerScript = getcallingscript()
                end)

                local callerInfo = Utils.GetCallerInfo(callerFunc, callerScript)
                ...
                ClonedFunctions.taskSpawn(function()
                    ...
                    pcall(function()
                        selfRef.Logger:Add({
                            ...
                            HookType = isNamecall and "__namecall" or "hookfunction",
                            Method = method,
                            Blocked = false,
                        })
                    end)
    ...
    self.OriginalNamecall = hookmetamethod(game, "__namecall", newcclosure(function(self, ...)
        ...
        if method == "FireServer" or method == "InvokeServer" or method == "Fire" or method == "Invoke" then
            if typeof(self) == "Instance" then
                selfRef.InHook = true
                local blocked = processCall(self, method, {...}, true)
    ...
    local function hookMethod(className, methodName)
        pcall(function()
            local inst = Instance.new(className)
            local original
            original = hookfunction(inst[methodName], newcclosure(function(self, ...)
                if selfRef.InHook or checkcaller() then
                    return original(self, ...)
                end
                if typeof(self) == "Instance" and self.ClassName == className then
                    selfRef.InHook = true
                    local blocked = processCall(self, methodName, {...}, false)
                    selfRef.InHook = false
                    if blocked then return end
                end
                return original(self, ...)
    ...
    hookMethod("RemoteEvent", "FireServer")
    hookMethod("UnreliableRemoteEvent", "FireServer")
    hookMethod("RemoteFunction", "InvokeServer")
    hookMethod("BindableEvent", "Fire")
    hookMethod("BindableFunction", "Invoke")
end
